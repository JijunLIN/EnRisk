...
class TotalEnsembleTrainer(pl.LightningModule):
    def __init__(
        self,
        ...
        self.history_steps = model.parallel[0].history_steps
...
def print_dict_structure(d: Dict[str, any], indent: int = 0) -> None:
    ...
    for key, value in d.items():
        # 打印当前键和值的类型
        print(" " * indent + f"Key: {key}, Value Type: {type(value).__name__} " + (f"Shape: {value.shape}" if type(value).__name__ == "Tensor" else ""))
        ...
        # 如果值是列表或元组，检查元素是否为字典
            ...
            for i, item in enumerate(value):
                print(" " * (indent + 4) + f"Index: {i}, Item Type: {type(item).__name__}")
                if isinstance(item, dict):
                    print_dict_structure(item, indent + 8)